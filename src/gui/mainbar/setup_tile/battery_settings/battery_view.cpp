...
static void battery_view_update_task( lv_task_t *task ) {
    ...
    lv_obj_align_to( battery_view_current_cap, lv_obj_get_parent( battery_view_current_cap ), LV_ALIGN_RIGHT_MID, -5, 0 );

    snprintf( temp, sizeof( temp ), "%dmAh", pmu_get_designed_battery_cap() );
    lv_label_set_text( battery_view_design_cap, temp );
    lv_obj_align_to( battery_view_design_cap, lv_obj_get_parent( battery_view_design_cap ), LV_ALIGN_RIGHT_MID, -5, 0 );

    snprintf( temp, sizeof( temp ), "%0.2fV", pmu_get_battery_voltage() / 1000 );
    lv_label_set_text( battery_view_voltage, temp );
    lv_obj_align_to( battery_view_voltage, lv_obj_get_parent( battery_view_voltage ), LV_ALIGN_RIGHT_MID, -5, 0 );

    snprintf( temp, sizeof( temp ), "%0.1fmA", pmu_get_battery_charge_current() );
    lv_label_set_text( charge_view_current, temp );
    lv_obj_align_to( charge_view_current, lv_obj_get_parent( charge_view_current ), LV_ALIGN_RIGHT_MID, -5, 0 );

    snprintf( temp, sizeof( temp ), "%0.1fmA", pmu_get_battery_discharge_current() );
    lv_label_set_text( discharge_view_current, temp );
    lv_obj_align_to( discharge_view_current, lv_obj_get_parent( discharge_view_current ), LV_ALIGN_RIGHT_MID, -5, 0 );

    snprintf( temp, sizeof( temp ), "%0.2fV", pmu_get_vbus_voltage() / 1000 );
    lv_label_set_text( vbus_view_voltage, temp );
    lv_obj_align_to( vbus_view_voltage, lv_obj_get_parent( vbus_view_voltage ), LV_ALIGN_RIGHT_MID, -5, 0 );
}
