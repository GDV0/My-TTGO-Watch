...
icon_t *app_register( const char* appname, const lv_img_dsc_t *icon, lv_event_cb_t event_cb ) {
    ...
    lv_obj_align_to( app->label , app->icon_cont, LV_ALIGN_OUT_BOTTOM_MID, 0, 0 );
    ...
    lv_obj_align_to( app->icon_indicator, app->icon_cont, LV_ALIGN_IN_TOP_RIGHT, 0, 0 );
...
void app_set_icon( icon_t *app, lv_obj_t *icon ) {
    ...
    lv_imgbtn_set_src( app->icon_img, LV_BTN_STATE_CHECKED_PRESSED, icon);
    lv_obj_reset_style_list( app->icon_img, LV_PART_MAIN );
    lv_obj_align_to( app->icon_img , app->icon_cont, LV_ALIGN_IN_TOP_LEFT, 0, 0 );
    lv_obj_invalidate( lv_scr_act() );
}
